...
/* ------------------------------------------------------------------ */
/*  generateTaxInvoicePdf(invoice) → triggers a download               */
/* ------------------------------------------------------------------ */
...
export function generateTaxInvoicePdf(invoice) {
  ...
  doc.setFontSize(11)
  doc.text('TAX INVOICE', MARGIN, 23)
  ...
  // Place of supply / currency strip
  if (regime.needsPlaceOfSupply || invoice.placeOfSupply) {
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(8)
    doc.setTextColor(...C_INK_500)
    const place = invoice.placeOfSupply === 'inter' ? 'Inter-state · IGST' : 'Intra-state · CGST + SGST'
    doc.text(`Place of supply: ${place}`, MARGIN, y)
  ...
  // Compute final col widths (fill remaining)
  ...
  invoice.items.forEach((it, i) => {
    ...
    const row = {
      ...
      taxR: `${Number(it.taxRate) || 0}%`,
      amt: formatNumber(ln.total),
    }
  ...
  y += 14
  if (totals.byRate.length > 0 && regime.id !== 'sales-us') {
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(7.5)
    doc.setTextColor(...C_INK_500)
  ...
  if (regime.id === 'gst-in') {
    ...
    doc.setTextColor(...C_INK_500); doc.text('Sales tax', totalsX, y)
    doc.setTextColor(...C_INK_700); doc.text(formatNumber(totals.sales), valueX, y, { align: 'right' })
    y += 14
  }
  ...
  doc.text(`${cur.code} ${formatNumber(totals.grandTotal)}`, valueX, y, { align: 'right' })
  y += 18
  ...
  doc.setFontSize(8.5)
  doc.setTextColor(...C_INK_700)
  const words = amountInWords(totals.grandTotal, cur.code)
  ...
  const fileName = `${(invoice.number || 'tax-invoice').replace(/[^a-z0-9-]+/gi, '_')}.pdf`
  doc.save(fileName)
}
