...
import {
  CURRENCIES, VALUATION_METHODS, STOCK_STATUSES, SAMPLE_DATA,
  findCurrency, findValuationMethod, findStockStatus, newItem,
  computeTotals,
  formatNumber, formatMoney, formatDate, todayISO,
} from '@/lib/inventoryValuation/compute'
import { generateInventoryValuationPdf } from '@/lib/inventoryValuation/generatePdf'
import { generateInventoryValuationXlsx } from '@/lib/inventoryValuation/generateXlsx'
...
function LiveDemoModal({ open, onClose }) {
  ...
  if (!open) return null
  return (
    <div role="dialog" aria-modal="true" aria-label="Live Inventory Valuation Report"
      onClick={onClose}
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-[1240px] px-6 py-20 md:py-28">
          <nav aria-label="Breadcrumb" className="mb-10 flex flex-wrap items-center gap-2 font-mono text-[11px] uppercase tracking-[0.12em] text-ink-500">
            ...
            <span className="text-ink-400">/</span>
            <span className="text-ink-950">Inventory Valuation Report</span>
          </nav>
...
function GeneratorPanel() {
  ...
  const method = useMemo(() => findValuationMethod(data.methodId), [data.methodId])
  const T = useMemo(() => computeTotals(data.items, data.methodId), [data.items, data.methodId])

  const setField = (k) => (v) => setData((s) => ({ ...s, [k]: v }))
  const reset = () => setData({ ...SAMPLE_DATA })
  const addItem = () => setData((s) => ({ ...s, items: [...s.items, newItem()] }))
  const updateItem = (id, patch) => setData((s) => ({ ...s, items: s.items.map((it) => it.id === id ? { ...it, ...patch } : it) }))
  const removeItem = (id) => setData((s) => ({ ...s, items: s.items.filter((it) => it.id !== id) }))

  const handlePdf  = async () => { try { setBusy('pdf');  generateInventoryValuationPdf(data) }  finally { setBusy(null) } }
  const handleXlsx = async () => { try { setBusy('xlsx'); generateInventoryValuationXlsx(data) } finally { setBusy(null) } }
    ...
    <aside className="relative">
      ...
      <div className="relative rounded-2xl border border-line bg-surface p-5 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <div className="inline-flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-ink-500">
              {method.label} · {data.items.length} lines
            </span>
          </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          <TextInput label="As-of date"  value={data.asOfDate}    onChange={setField('asOfDate')} />
        </div>
        <div className="mt-2 grid grid-cols-[1fr_92px] gap-2">
          <TextInput label="Company" value={data.company} onChange={setField('company')} />
        ...
        <div className="mt-2 grid grid-cols-[1fr_140px] gap-2">
          <TextInput label="Warehouse / location" value={data.warehouse} onChange={setField('warehouse')} />
          <SelectInput label="Method" value={data.methodId} onChange={setField('methodId')}
            options={VALUATION_METHODS.map((m) => ({ value: m.id, label: m.label }))} />
        </div>
        ...
        {/* Items */}
        <div className="mb-2 flex items-center justify-between">
          <span className="block font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-accounting">
            Stock lines ({data.items.length})
          </span>
          <button type="button" onClick={addItem}
            className="inline-flex items-center gap-1 rounded-full border border-accounting/40 bg-accounting-bg px-2.5 py-1 font-mono text-[9.5px] uppercase tracking-[0.08em] text-accounting hover:bg-accounting/10">
            <Plus size={10} /> Add line
          </button>
        </div>
        <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
          {data.items.map((it) => (
            <div key={it.id} className="rounded-lg border border-line bg-canvas p-2">
              <div className="grid grid-cols-[1fr_1fr_28px] gap-1.5">
                <TextInput label="SKU" value={it.sku} onChange={(v) => updateItem(it.id, { sku: v })} mono />
                <TextInput label="Category" value={it.category} onChange={(v) => updateItem(it.id, { category: v })} />
                <button type="button" onClick={() => removeItem(it.id)} aria-label="Remove"
                  className="mt-5 flex h-7 w-7 items-center justify-center rounded-md border border-line bg-paper font-mono text-[10px] text-ink-500 hover:border-crimson-400 hover:text-crimson-400">×</button>
              </div>
              <div className="mt-1.5">
                <TextInput label="Item name" value={it.name} onChange={(v) => updateItem(it.id, { name: v })} />
              </div>
              <div className="mt-1.5 grid grid-cols-3 gap-1.5">
                <NumberInput label="Qty"        value={it.qty}       onChange={(v) => updateItem(it.id, { qty: v })} />
                <NumberInput label="Unit cost"  value={it.unitCost}  onChange={(v) => updateItem(it.id, { unitCost: v })} />
                <NumberInput label="Unit price" value={it.unitPrice} onChange={(v) => updateItem(it.id, { unitPrice: v })} />
              </div>
              <div className="mt-1.5 grid grid-cols-2 gap-1.5">
                <NumberInput label="NRV (lower-of test)" value={it.nrv} onChange={(v) => updateItem(it.id, { nrv: v })} />
                <SelectInput label="Status" value={it.status} onChange={(v) => updateItem(it.id, { status: v })}
                  options={STOCK_STATUSES.map((s) => ({ value: s.id, label: s.label }))} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-lg border border-line bg-canvas px-3 py-2.5">
            <p className="m-0 font-mono text-[9px] uppercase tracking-[0.1em] text-ink-500">Total value</p>
            <p className="m-0 mt-1 font-mono text-[13px] font-semibold text-accounting">{formatNumber(T.totalValue)}</p>
            <p className="m-0 font-mono text-[9px] text-ink-500">{cur.code} · {method.label}</p>
          </div>
          <div className="rounded-lg border border-line bg-canvas px-3 py-2.5">
            <p className="m-0 font-mono text-[9px] uppercase tracking-[0.1em] text-ink-500">Retail value</p>
            <p className="m-0 mt-1 font-mono text-[13px] font-semibold text-ink-950">{formatNumber(T.totalRetail)}</p>
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          <div className="rounded-lg border border-line bg-canvas px-3 py-2">
            ...
            <p className="m-0 mt-0.5 font-mono text-[11px] font-semibold text-ink-950">{T.lines}</p>
          </div>
          <div className="rounded-lg border border-line bg-canvas px-3 py-2">
            <p className="m-0 font-mono text-[8.5px] uppercase tracking-[0.1em] text-ink-500">Qty</p>
            <p className="m-0 mt-0.5 font-mono text-[11px] font-semibold text-ink-950">{formatNumber(T.qty)}</p>
          </div>
          <div className="rounded-lg border border-line bg-canvas px-3 py-2">
            <p className="m-0 font-mono text-[8.5px] uppercase tracking-[0.1em] text-ink-500">Margin %</p>
            <p className="m-0 mt-0.5 font-mono text-[11px] font-semibold text-accounting">{formatNumber(T.potentialMargin)}%</p>
          </div>
        </div>
              ...
              <tbody className="font-mono">
                {T.categories.map((c, i) => (
                  <tr key={i} className="border-t border-line">
                    <td className="py-1 text-ink-700 truncate max-w-[160px]">{c.category}</td>
                    <td className="py-1 text-right text-ink-700">{formatNumber(c.qty)}</td>
                    <td className="py-1 text-right text-accounting">{formatNumber(c.totalValue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="mt-3 rounded-lg border border-accounting/40 bg-ink-950 px-3 py-3">
          <div className="flex items-center justify-between">
            <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-accounting">Inventory value</span>
            <span className="font-mono text-[10px] uppercase tracking-[0.08em] text-ink-500">
              {formatDate(data.asOfDate) || 'today'}
            </span>
          </div>
          <div className="mt-1 text-right font-mono text-[18px] font-semibold text-paper">
            {formatMoney(T.totalValue, data.currencyCode)}
          </div>
        </div>

        <div className="mt-3">
          <TextareaInput label="Notes (appear in PDF)" value={data.notes} onChange={setField('notes')}
            placeholder="Counting method, write-down basis, exclusions…" rows={2} />
        </div>

        <button type="button" onClick={handlePdf} disabled={busy !== null}
          className="btn btn-primary btn-lg mt-4 w-full disabled:cursor-wait disabled:opacity-70">
          {busy === 'pdf' ? 'Generating…' : 'Generate Inventory Valuation PDF'}
          <ArrowRight size={14} />
        </button>
...
export default function InventoryValuationReportTool() {
  return (
    <>
      <ToolHero />
      <PreviewSection />
