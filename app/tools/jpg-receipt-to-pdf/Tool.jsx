...
function LiveDemoModal({ open, onClose }) {
  ...
  if (!open) return null
  return (
    <div role="dialog" aria-modal="true" aria-label="JPG Receipt to PDF"
      onClick={onClose}
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-[1240px] px-6 py-20 md:py-28">
          <nav aria-label="Breadcrumb" className="mb-10 flex flex-wrap items-center gap-2 font-mono text-[11px] uppercase tracking-[0.12em] text-ink-500">
            ...
            <span className="text-ink-400">/</span>
            <span className="text-ink-950">JPG Receipt to PDF</span>
          </nav>
...
function ImageDrop({ image, onPick, onClear }) {
  ...
  return (
    ...
    >
      ...
      />
      <ImageToPdfIcon />
      <p className="m-0 font-mono text-[11px] uppercase tracking-[0.1em] text-ink-700">Drop a JPG / PNG receipt or click to browse</p>
  ...
  return (
    <div className="rounded-md border border-line bg-paper">
      ...
      <div className="p-5">
        ...
        <p className="m-0 mt-2 font-mono text-[7px] text-ink-500">IMG_2031.jpg</p>
...
const STEPS = [
  ['01', 'Drop the image',         'Drag a JPG or PNG of a single receipt from your phone or scanner. The tool reads its dimensions in the browser — nothing uploads.'],
...
const FAQS = [
  { q: 'How is this different from "Receipt Image to PDF"?',              a: 'Receipt Image to PDF is the multi-receipt batch packer — drop 30 phone photos and get one PDF with a cover sheet, manifest, and one receipt per page. JPG Receipt to PDF is the single-receipt fast lane: one image in, one PDF out, minimal config. Use the batch tool for end-of-month expense packs; use this one when you need to PDF a single receipt fast.' },
  { q: 'What image formats are supported?',                              a: 'JPG / JPEG and PNG work everywhere. WebP works in most browsers. HEIC (iPhone default) works only in browsers that decode HEIC natively — Safari on macOS / iOS. On Chrome / Firefox you may need to convert HEIC to JPG first (most phones do this when emailing or AirDropping).' },
  { q: 'Why is the output bigger than my source JPG?',                   a: 'PDF adds page metadata and the image gets re-embedded (not just renamed). For a small JPG, the PDF is typically 10–30% larger. If file size matters, run the output through the Compress Invoice PDFs tool — typically shrinks back below the source JPG with no visible quality loss.' },
...
export default function JpgReceiptToPdfTool() {
  return (
    <>
      <ToolHero />
      <PreviewSection />
