/* ------------------------------------------------------------------ */
/*  Service Agreement (MSA + SoW) — helpers + constants                */
/* ------------------------------------------------------------------ */
...
export const FEE_STRUCTURES = [
  { id: 'fixed',     label: 'Fixed fee per SoW' },
...
export function countSections(data) {
  ...
  if (data.includeSoW) n++
  if (data.includeServiceLevels) n++
  if (data.includeChangeOrders) n++
  if (data.includeIP) n++
