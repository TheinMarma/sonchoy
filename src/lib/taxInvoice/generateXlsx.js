...
export function generateTaxInvoiceXlsx(invoice) {
  ...
  const itemRows = invoice.items.map((it, i) => {
    ...
    base.push(Number(it.qty) || 0, Number(it.rate) || 0, ln.taxable, `${Number(it.taxRate) || 0}%`, ln.tax, ln.total)
    return base
  })
  ...
  const fileName = `${(invoice.number || 'tax-invoice').replace(/[^a-z0-9-]+/gi, '_')}.xlsx`
