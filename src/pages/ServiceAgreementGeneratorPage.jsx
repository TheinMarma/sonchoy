...
} from '../lib/serviceAgreement/compute'
import { generateServiceAgreementPdf } from '../lib/serviceAgreement/generatePdf'
import { generateServiceAgreementDocx } from '../lib/serviceAgreement/generateDocx'
...
function LiveDemoModal({ open, onClose }) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Live Service Agreement Generator"
      onClick={onClose}
...
const HERO_STATS = [
  ['MSA+SoW', 'Master + Statement of Work'],
  ['12',      'Standard clauses'],
  ['PDF+',    'DOCX (editable)'],
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-[1240px] px-6 py-20 md:py-28">
          <nav aria-label="Breadcrumb" className="mb-10 flex flex-wrap items-center gap-2 font-mono text-[11px] uppercase tracking-[0.12em] text-ink-500">
            ...
            <span className="text-ink-400">/</span>
            <span className="text-ink-950">Service Agreement Generator</span>
          </nav>
...
const INITIAL = {
  agreementTitle: 'Brand & Marketing Services MSA',
  agreementNumber: 'MSA-2026-0042',
  ...
  // SoW
  includeSoW: true,
  sowTitle: 'SoW #1 — Brand Identity Sprint',
  sowSummary: 'Provider shall design and deliver a complete brand identity system for Client, including logo, type system, colour palette, and brand guidelines, with handover scheduled for Q4 2026.',
  sowDeliverables: [
    ...
    { id: 4, description: 'Source files on project completion (.ai, .indd, .fig)' },
  ],
  sowStartDate: todayISO(),
  sowEndDate: '2026-12-31',
  sowMilestones: [
    { id: 5, description: '40% on signing',          amount: 12800, dueDate: todayISO() },
    { id: 6, description: '40% on midpoint review',  amount: 12800, dueDate: '2026-09-15' },
    { id: 7, description: '20% on final delivery',   amount: 6400,  dueDate: '2026-12-15' },
  ],
  ...
  serviceLevelDescription: 'Provider shall respond to Client communications within one (1) business day. Initial drafts shall be delivered per the SoW timeline. Two rounds of revisions per deliverable are included; additional revisions shall be handled via change order.',
...
function GeneratorPanel() {
  ...
  const sowTotal = useMemo(() => computeMilestoneTotal(data.sowMilestones), [data.sowMilestones])
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      sowDeliverables: cleanLines(data.sowDeliverables),
      sowMilestones: cleanLines(data.sowMilestones).map((m) => ({
        ...m,
  ...
  const handlePdf  = async () => { try { setBusy('pdf');  generateServiceAgreementPdf(buildPayload()) }  finally { setBusy(null) } }
  const handleDocx = async () => { try { setBusy('docx'); await generateServiceAgreementDocx(buildPayload()) } finally { setBusy(null) } }
    ...
    <aside className="relative">
      ...
      <div className="relative rounded-2xl border border-line bg-surface p-5 shadow-2xl">
        ...
        <TextInput
          ...
          onChange={setField('agreementTitle')}
        ...
        {/* Services */}
        <TextareaInput
          label="Services description (master scope)"
          value={data.servicesDescription}
          onChange={setField('servicesDescription')}
          placeholder="What Provider does — generally."
          rows={3}
        />
        ...
        {/* SoW */}
        <ToggleRow
          label="Include initial Statement of Work"
          desc="Attach SoW #1 to the MSA at signing"
          checked={data.includeSoW}
          onChange={setField('includeSoW')}
        />
        {data.includeSoW && (
          <div className="mt-3 rounded-lg border border-contracts/20 bg-contracts-bg/30 p-3 space-y-2">
            <TextInput
              label="SoW title"
              value={data.sowTitle}
              onChange={setField('sowTitle')}
              placeholder="SoW #1 — Project name"
            />
            <TextareaInput
              label="SoW summary"
              value={data.sowSummary}
              onChange={setField('sowSummary')}
              rows={2}
            />
            <StringList
              title="Deliverables"
              lines={data.sowDeliverables}
              setLines={setLines('sowDeliverables')}
              placeholder="What gets delivered"
              accent
            />
            <div className="grid grid-cols-2 gap-2">
              <DateInput label="Start date" value={data.sowStartDate} onChange={setField('sowStartDate')} />
              <DateInput label="Target end"  value={data.sowEndDate}   onChange={setField('sowEndDate')} />
            </div>
            <MilestoneList
              lines={data.sowMilestones}
              setLines={setLines('sowMilestones')}
            ...
            <div className="flex items-center justify-between rounded-md border border-line bg-canvas px-3 py-1.5">
              <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-ink-500">SoW total</span>
              <span className="font-mono text-[12px] font-semibold text-contracts">
                {formatMoney(sowTotal, data.feeCurrency)}
              </span>
            </div>
          </div>
        )}
        ...
        />
        {(fee.id === 'hourly' || fee.id === 'retainer') && (
          <div className="mt-2 grid grid-cols-[1fr_92px] gap-2">
            <NumberInput
              label={fee.id === 'hourly' ? 'Hourly rate' : 'Monthly retainer'}
        ...
        <div className="space-y-2">
          <ToggleRow
            ...
            desc="Response times, revisions per deliverable"
            checked={data.includeServiceLevels}
            onChange={setField('includeServiceLevels')}
          />
          {data.includeServiceLevels && (
            <div className="ml-3">
              <TextareaInput
                label="Service level description"
                value={data.serviceLevelDescription}
                onChange={setField('serviceLevelDescription')}
                rows={2}
              />
            </div>
          )}
          <ToggleRow
            ...
            desc="Written change requests required for scope changes"
            checked={data.includeChangeOrders}
            onChange={setField('includeChangeOrders')}
          />
          <ToggleRow
          ...
          <ToggleRow
            ...
            desc="Cap aggregate liability per SoW"
        ...
        <div className="rounded-lg border border-line bg-canvas p-3">
          ...
          <div className="space-y-1 text-[11px]">
            {[
              ...
              ['Fee',         fee.id === 'milestone' ? `${cur.code} ${formatNumber(sowTotal)} (SoW milestones)` : `${cur.code} ${formatNumber(Number(data.feeAmount) || 0)} (${fee.label.toLowerCase()})`],
              ['SoW',         data.includeSoW ? `${data.sowDeliverables.length} deliverables · ${data.sowMilestones.length} milestones` : 'None included'],
        ...
        <div className="mt-3 rounded-lg border border-contracts/40 bg-ink-950 px-3 py-3">
          ...
          <div className="mt-1 text-right font-mono text-[18px] font-semibold text-paper">
            {data.includeSoW && fee.id === 'milestone'
              ? formatMoney(sowTotal, data.feeCurrency)
              : (fee.id === 'hourly' || fee.id === 'retainer')
        ...
        >
          {busy === 'pdf' ? 'Generating…' : 'Generate Agreement PDF'}
          <ArrowRight size={14} />
        </button>
  ...
  return (
    <div className="rounded-md border border-line bg-paper p-6">
      <div className="text-center">
        <p className="m-0 font-mono text-[8px] uppercase tracking-[0.16em] text-ink-500">MASTER SERVICE AGREEMENT</p>
        <p className="m-0 mt-2 text-[18px] font-bold tracking-[-0.01em] text-ink-950">Brand &amp; Marketing Services MSA</p>
        <p className="m-0 mt-1 text-[10px] text-ink-500">MSA-2026-0042 · Effective 30 Jun 2026</p>
      ...
      <p className="m-0 mt-4 text-[11px]">
        <span className="font-bold text-contracts">1.</span> <span className="font-bold text-ink-950">SERVICES</span>
      </p>
      ...
      <p className="m-0 mt-3 text-[11px]">
        <span className="font-bold text-contracts">2.</span> <span className="font-bold text-ink-950">STATEMENT OF WORK</span>
      </p>
      <p className="m-0 mt-1 text-[10px] font-bold text-ink-950">SoW #1 — Brand Identity Sprint</p>
      <p className="m-0 mt-1 text-[9.5px] leading-[1.5] text-ink-700">
        Provider shall design and deliver a complete brand identity system for Client, including logo, type system, colour palette…
      </p>
      <p className="m-0 mt-2 text-[9.5px] font-bold text-ink-950">Milestones:</p>
      <ul className="m-0 mt-0.5 list-none space-y-0.5 pl-3">
        {[
          '40% on signing: USD 12,800.00',
          '40% on midpoint: USD 12,800.00',
          '20% on final delivery: USD 6,400.00',
        ].map((t) => (
          <li key={t} className="text-[9.5px] text-ink-700">• {t}</li>
        ))}
      </ul>
        ...
        <div className="grid grid-cols-1 items-stretch gap-3 lg:grid-cols-[1fr_auto_1fr]">
          <div className="rounded-2xl border border-line bg-paper p-1">
            <div className="rounded-xl bg-surface p-5">
              ...
              <div className="space-y-2">
                {[
                  ['Title',          'Brand & Marketing Services MSA'],
                  ['Parties',        'Sonchoy Studio ↔ Northwind Books'],
                  ['Term',           'Ongoing · 30-day notice'],
                  ['Fee',            'Milestone-based · USD 32,000 SoW#1'],
                  ['SoW',            '4 deliverables · 3 milestones'],
                  ['IP',             'Assigned to Client on full payment'],
                  ['Service levels', '1 business day response · 2 revisions'],
                ].map(([k, v]) => (
              ...
              <div className="mt-3 flex items-center justify-between rounded-lg border border-contracts/30 bg-ink-950 px-3 py-2.5">
                <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-contracts">SoW value</span>
...
const STEPS = [
  ['01', 'MSA — the relationship', 'Parties, scope of services, payment terms, IP, confidentiality, liability. Sign this once; it covers every project that follows.'],
  ['02', 'SoW — the project',      'Project-specific deliverables, timeline, milestones. Each new project gets its own SoW that attaches to the MSA.'],
  ['03', 'Download and sign',      'PDF + editable DOCX. The MSA + initial SoW is one document; subsequent SoWs are short addenda referencing the master.'],
]
      ...
      <div className="mx-auto max-w-[1240px] px-6">
        <div className="mb-12 grid grid-cols-1 items-end gap-10 md:grid-cols-2 md:gap-16">
          <div>
            ...
            <SectionTitle>
              Sign once, reuse for{' '}
              <em className="font-serif font-normal italic text-crimson-300">every project.</em>
            </SectionTitle>
          </div>
...
const FEATURES = [
  { title: 'MSA + SoW structure',     desc: 'Master terms cover the relationship; project terms live in SoWs. Standard for serious B2B engagements.' },
  { title: '4 fee structures',         desc: 'Fixed per SoW, hourly rate, monthly retainer, or milestone-based. Mix and match across SoWs.' },
  { title: 'Service-level commitments', desc: 'Response times, revision rounds, escalation. Per-SoW SLAs supported via change orders.' },
  { title: 'Change-order process',     desc: 'Standard process for scope/timeline/fee changes — protects Provider from scope creep without renegotiation.' },
  { title: 'IP, confidentiality, liability', desc: 'Three IP modes, mutual confidentiality, liability cap per SoW, indemnity — same as the Client Contract Generator.' },
  { title: 'PDF + editable DOCX',      desc: 'Print-ready PDF for signing, plus a fully editable .docx for redlining with counsel or your counter-party.' },
]
    ...
    <section className="bg-canvas py-24">
      <div className="mx-auto max-w-[1240px] px-6">
        <div className="mx-auto mb-12 max-w-[720px] text-center">
          <Eyebrow className="mb-4">03 — Built for ongoing relationships</Eyebrow>
          <SectionTitle>
            Every clause your{' '}
            <em className="font-serif font-normal italic text-crimson-300">procurement team</em> expects.
          </SectionTitle>
        </div>
...
const FAQS = [
  { q: 'When should I use an MSA + SoW instead of a single contract?',  a: 'When you expect multiple projects over time with the same client. Negotiate the MSA terms (IP, liability, payment) once with counsel review, then each new project gets a quick SoW that references the MSA. Saves weeks of legal time per project.' },
  { q: 'What goes in the MSA vs the SoW?',                              a: 'MSA: relationship-level terms — parties, IP, confidentiality, liability cap, governing law, payment terms. SoW: project-level specifics — deliverables, timeline, milestones, project-specific service levels. Don\'t put project terms in the MSA; you\'ll regret it on project #2.' },
  { q: 'Can I attach the SoW separately later?',                        a: 'Yes — sign the MSA without any SoW, then add SoWs over time. The tool will toggle the SoW section off if you uncheck "Include initial SoW". Future SoWs can be short standalone documents that reference the MSA by number and date.' },
  { q: 'Why do liability caps work per-SoW?',                           a: 'A typical liability cap (1× fees) makes sense per project, not across an entire multi-year relationship. The MSA caps liability at "fees paid under the applicable SoW in the 12 months preceding the claim" — so a 200K SoW caps at 200K, not at all SoWs aggregated.' },
  { q: 'Is this enforceable internationally?',                          a: 'Generally yes — MSAs are enforceable in most common-law and civil-law jurisdictions. The governing-law clause picks which courts apply. For high-value cross-border deals, get local counsel to review the DOCX before signing.' },
  { q: 'Output formats?',                                               a: 'PDF (multi-page, numbered sections, signature blocks on the last page, page footers) and .docx (fully editable, same content, ready for counsel redlines or counter-party negotiation).' },
]
    ...
    <section className="py-24">
      <div className="mx-auto max-w-[1240px] px-6">
        <div className="mx-auto mb-12 max-w-[680px] text-center">
          ...
          <SectionTitle>
            Everything about{' '}
            <em className="font-serif font-normal italic text-crimson-300">service agreements.</em>
          </SectionTitle>
        </div>
...
const RELATED = [
  { name: 'Client Contract Generator', desc: 'Single-project engagement contracts.',          Icon: ContractIcon, label: 'CONTRACTS', path: '/tools/client-contract-generator' },
...
export default function ServiceAgreementGeneratorPage() {
  return (
    <>
      <ToolHero />
      <PreviewSection />
